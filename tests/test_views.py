...
class SeriesViewTest(FictionOutlineViewTestCase):
    ...
    def test_view_shows_only_users_series(self):
        ...
        with self.login(username=self.user3.username):
            ...
            self.assertResponseContains("You do not have any series defined yet.", html=False)
...
class SeriesCreateViewTest(FictionOutlineViewTestCase):
    ...
    def test_series_creation(self):
        ...
        with self.login(username=self.user1.username):
            self.post('fiction_outlines:series_create', data={
    ...
    def test_object_permissions_honored(self):
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                ...
                self.post('fiction_outlines:series_update', series=self.s1.pk, data={'title': 'war of cats'})
                self.response_302()
...
class CharacterListViewTest(FictionOutlineViewTestCase):
    ...
    def test_login_required(self):
            ...
            for character in [self.c1, self.c2, self.c3, self.c4]:
                self.assertLoginRequired('fiction_outlines:character_instance_list', character=character.pk)
    ...
    def test_character_restricted_by_user(self):
        ...
        for user in [self.user1, self.user2]:
            with self.login(username=user.username):
                ...
                assert len(character_list) == Character.objects.filter(user=user).count()
                for character in character_list:
                    assert character.user == user
        with self.login(username=self.user3.username):
            self.get('fiction_outlines:character_list')
            self.assertResponseContains("You don't have any characters defined yet.", html=False)
    ...
    def test_character_instance_view_restriction(self):
        ...
        with self.login(username=self.user3.username):
            for character in [self.c1, self.c2, self.c3, self.c4]:
                print("Now evaluating character: %s <%s>" % (character.name, character.pk))
                self.get('fiction_outlines:character_instance_list', character=character.pk)
                self.response_302()
        with self.login(username=self.user2.username):
            ...
            for character in [self.c3, self.c4]:
                self.assertGoodView('fiction_outlines:character_instance_list', character=character.pk)
                charints = self.get_context("character_instance_list")
                assert len(charints) == CharacterInstance.objects.filter(character=character).count()
...
class CharacterCreateTestCase(FictionOutlineViewTestCase):
    ...
    def test_character_create(self):
        ...
        with self.login(username=self.user1.username):
            ...
            assert 'Mary Sue' == Character.objects.filter(user=self.user1).latest('created').name
...
class CharacterDetailTestCase(FictionOutlineViewTestCase):
    ...
    def test_requries_login(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:character_detail', character=self.c1.pk)
    ...
    def test_normal_view(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:character_detail', character=self.c1.pk)
...
class CharacterUpdateTestCase(FictionOutlineViewTestCase):
    ...
    def test_requires_login(self):
        ...
        '''
        self.assertLoginRequired('fiction_outlines:character_update', character=self.c1.pk)
    ...
    def test_access_requires_object_permission(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                ...
                self.response_302()
                self.post('fiction_outlines:character_update', character=self.c1.pk, data={
                    'name': 'Doctor Nick',
                ...
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                assert self.c1.name == Character.objects.get(pk=self.c1.pk).name == "John Doe"
    ...
    def test_normal_workflow(self):
        ...
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:character_update', character=self.c1.pk)
            self.assertInContext('form')
            self.post('fiction_outlines:character_update', character=self.c1.pk, data={
                'name': 'Doctor Nick',
                'description': 'Hi everybody!',
...
class CharacterDeleteViewTest(FictionOutlineViewTestCase):
    ...
    def test_requires_login(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:character_delete', character=self.c1.pk)
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:character_delete', character=self.c1.pk)
            self.assertInContext('character')
...
class CharacterInstanceDetailTest(FictionOutlineViewTestCase):
    ...
    def test_requires_login(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:character_instance_detail',
                                 character=self.c1.pk, instance=self.c1int.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:character_instance_detail', character=self.c1.pk, instance=self.c1int.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:character_instance_detail',
                                character=self.c1.pk, instance=self.c1int.pk)
...
class CharacterInstanceUpdateTest(FictionOutlineViewTestCase):
    ...
    def test_requires_login(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:character_instance_update',
                                 character=self.c1.pk, instance=self.c1int.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:character_instance_update',
                         character=self.c1.pk, instance=self.c1int.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                assert not self.c1int.main_character
                self.post('fiction_outlines:character_instance_update',
                          character=self.c1.pk, instance=self.c1int.pk, data={
                              'outline': self.o1.pk,
                              'main_character': True,
                              'pov_character': False,
                              'protagonist': False,
    ...
    def test_valid_form(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:character_instance_update',
                                character=self.c1.pk, instance=self.c1int.pk)
            self.assertInContext('form')
            assert self.c1int == self.get_context('character_instance')
            self.post('fiction_outlines:character_instance_update',
...
class CharacterInstanceDeleteTest(FictionOutlineViewTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:character_instance_delete',
                                 character=self.c1.pk, instance=self.c1int.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:character_instance_delete',
                         character=self.c1.pk, instance=self.c1int.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:character_instance_delete',
                          character=self.c1.pk, instance=self.c1int.pk)
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:character_instance_delete',
                                character=self.c1.pk, instance=self.c1int.pk)
            assert self.c1int == self.get_context('character_instance')
            self.post('fiction_outlines:character_instance_delete',
                      character=self.c1.pk, instance=self.c1int.pk)
...
class OutlineCreateTestCase(FictionOutlineViewTestCase):
    ...
    def test_form_submittal_with_invalid_series(self):
        ...
        with self.login(username=self.user1.username):
            ...
            self.post('fiction_outlines:outline_create', data={
                'title': 'My Opus',
    ...
    def test_valid_form_submittal(self):
        ...
        with self.login(username=self.user1.username):
            ...
            assert 'My Opus' == Outline.objects.filter(user=self.user1).latest('created').title
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                ...
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:outline_update', outline=self.o1.pk, data={
...
class LocationListViewTestCase(FictionOutlineViewTestCase):
    ...
    def test_login_required(self):
        ...
        for loc in [self.l1, self.l2, self.l3, self.l4]:
            self.assertLoginRequired('fiction_outlines:location_instance_list', location=loc.pk)
    ...
    def test_locations_restricted_by_user(self):
        ...
        with self.login(username=self.user3.username):
            self.get('fiction_outlines:location_list')
            self.assertResponseContains("You don't have any locations defined yet.", html=False)
    ...
    def test_location_instance_list_limited_by_object_permission(self):
        ...
        with self.login(username=self.user2.username):
            for loc in [self.l3, self.l4]:
                self.assertGoodView('fiction_outlines:location_instance_list', location=loc.pk)
                instances = self.get_context('location_instance_list')
                assert len(instances) == LocationInstance.objects.filter(location=loc).count()
...
class LocationDetailViewTest(FictionOutlineViewTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:location_detail', location=self.l1.pk)
...
class LocationUpdateTest(FictionOutlineViewTestCase):
    ...
    def test_requires_login(self):
        ...
        '''
        self.assertLoginRequired('fiction_outlines:location_update', location=self.l1.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                ...
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:location_update', location=self.l1.pk, data={
...
class LocationDeleteViewTest(FictionOutlineViewTestCase):
    ...
    def test_requires_login(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:location_delete', location=self.l1.pk)
...
class LocationInstanceDetailTest(FictionOutlineViewTestCase):
    ...
    '''
    def test_login_required(self):
        self.assertLoginRequired('fiction_outlines:location_instance_detail',
                                 location=self.l1.pk, instance=self.l1int.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:location_instance_detail',
                         location=self.l1.pk, instance=self.l1int.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
    ...
    def test_normal_workflow(self):
        ...
        with self.login(username=self.user1):
            self.assertGoodView('fiction_outlines:location_instance_detail',
                                location=self.l1.pk, instance=self.l1int.pk)
...
class LocationInstanceUpdateTest(FictionOutlineViewTestCase):
    ...
    def test_login_required(self):
        ...
        Login is required.
        '''
        self.assertLoginRequired('fiction_outlines:location_instance_update',
                                 location=self.l1.pk, instance=self.l1int.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user3, self.user2]:
            with self.login(username=user.username):
                self.get('fiction_outlines:location_instance_update',
                         location=self.l1.pk, instance=self.l1int.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:location_instance_update',
                          location=self.l1.pk, instance=self.l1int.pk, data={
                              'outline': self.o2.pk,
                          })
    ...
    def test_valid_post(self):
        with self.login(username=self.user1.username):
            ...
            o4.save()
            self.assertGoodView('fiction_outlines:location_instance_update',
                                location=self.l1.pk, instance=self.l1int.pk)
            self.assertInContext('form')
            assert self.l1int == self.get_context('location_instance')
            self.post('fiction_outlines:location_instance_update',
...
class ArcDetailTest(FictionOutlineViewTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:arc_detail', outline=self.o1.pk, arc=self.arc1.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arc_detail', outline=self.o1.pk, arc=self.arc1.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:arc_detail', outline=self.o1.pk, arc=self.arc1.pk)
...
class ArcCreateTest(FictionOutlineViewTestCase):
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                ...
                self.post('fiction_outlines:arc_create', outline=self.o1.pk, data={
    ...
    def test_allowed_workflow(self):
        ...
        with self.login(username=self.user1.username):
            ...
            assert count_of_arcs_outline < Arc.objects.filter(outline=self.o1).count()
            assert 'Retrieve the missing anaconda' == Arc.objects.filter(outline=self.o1).latest('created').name
...
class ArcUpdateTest(FictionOutlineViewTestCase):
    ...
    '''
    def test_login_required(self):
        self.assertLoginRequired('fiction_outlines:arc_update', outline=self.o1.pk, arc=self.arc1.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arc_update', outline=self.o1.pk, arc=self.arc1.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:arc_update', outline=self.o1.pk, arc=self.arc1.pk, data={
                    'name': 'Cha-cha-changes',
                    'mace_type': 'event',
                })
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:arc_update', outline=self.o1.pk, arc=self.arc1.pk)
            self.assertInContext('form')
            assert self.arc1 == self.get_context('arc')
...
class ArcDeleteTest(FictionOutlineViewTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:arc_delete', outline=self.o1.pk, arc=self.arc1.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arc_delete', outline=self.o1.pk, arc=self.arc1.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:arc_delete', outline=self.o1.pk, arc=self.arc1.pk)
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:arc_delete', outline=self.o1.pk, arc=self.arc1.pk)
            assert self.arc1 == self.get_context('arc')
            self.post('fiction_outlines:arc_delete', outline=self.o1.pk, arc=self.arc1.pk)
            with pytest.raises(ObjectDoesNotExist):
...
class ArcnodeDetailViewTest(ArcNodeAbstractTestCase):
    ...
    def test_login_request(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:arcnode_detail', outline=self.o1.pk,
                                 arc=self.arc1.pk, arcnode=self.node_to_test.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arcnode_detail',
                         outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
    ...
    def test_object_owner_access(self):
        ...
        with self.login(username=self.user1.username):
            ...
            self.assertGoodView('fiction_outlines:arcnode_detail', outline=self.o1.pk, arc=self.arc1.pk,
...
class ArcNodeUpdateViewTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:arcnode_update',
                                 outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arcnode_update',
                         outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:arcnode_update',
                          outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk,
                          data={
                ...
                assert self.node_to_test == ArcElementNode.objects.get(pk=self.node_to_test.pk)
    ...
    def test_invalid_submission(self):
        ...
        with self.login(username=self.user1.username):
            self.post('fiction_outlines:arcnode_update',
                      outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, data={
                          'name': 'Chapter 1',
            ...
            self.response_200()
            assert len(self.get_context('form').errors) > 0
            assert self.node_to_test.arc_element_type == ArcElementNode.objects.get(
                pk=self.node_to_test.pk).arc_element_type
    ...
    def test_valid_form_submission(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:arcnode_update',
                                outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
            self.assertInContext('form')
            assert self.node_to_test == self.get_context('arcnode')
            self.post('fiction_outlines:arcnode_update',
...
class ArcNodeMoveTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:arcnode_move', outline=self.o1.pk,
                                 arc=self.arc1.pk, arcnode=self.node_to_test.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arcnode_move', outline=self.o1.pk,
                         arc=self.arc1.pk, arcnode=self.node_to_test.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:arcnode_move', outline=self.o1.pk,
                          arc=self.arc1.pk, arcnode=self.node_to_test.pk,
                          data={
    ...
    def test_valid_move(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:arcnode_move', outline=self.o1.pk,
                                arc=self.arc1.pk, arcnode=self.node_to_test.pk,
    ...
    def test_invalid_move(self):
        ...
        '''
        with self.login(username=self.user1.username):
            arc2 = self.o1.create_arc(name='Totally different arc', mace_type='character')
            invalid_target_node = arc2.arc_root_node.get_children()[2]
            self.post('fiction_outlines:arcnode_move', outline=self.o1.pk,
                      arc=self.arc1.pk, arcnode=self.node_to_test.pk,
                      data={
                          '_ref_node_id': invalid_target_node.pk,
                          '_position': 'right',
                      })
            self.response_200()
            assert len(self.get_context('form').errors) > 0
...
class ArcNodeDeleteTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:arcnode_delete',
                                 outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arcnode_delete',
                         outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:arcnode_delete',
                          outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:arcnode_delete',
                                outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
            assert self.node_to_test == self.get_context('arcnode')
            self.assertResponseContains("You cannot delete the hook of an arc.", html=False)
            self.post('fiction_outlines:arcnode_delete',
                      outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk)
            ...
            self.assertGoodView('fiction_outlines:arcnode_delete',
                                outline=self.o1.pk, arc=self.arc1.pk, arcnode=reso.pk)
            self.assertResponseContains("You cannot delete the resolution of an arc.", html=False)
            self.post('fiction_outlines:arcnode_delete',
                      outline=self.o1.pk, arc=self.arc1.pk, arcnode=reso.pk)
            self.response_200()
            ...
            self.assertGoodView('fiction_outlines:arcnode_delete',
                                outline=self.o1.pk, arc=self.arc1.pk, arcnode=pt1.pk)
...
class ArcNodeCreateTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:arcnode_create',
                                 outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='addchild')
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:arcnode_create',
                         outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='addchild')
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:arcnode_create',
                          outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='addchild',
                          data={
    ...
    def test_authorized_creations(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:arcnode_create',
                                outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='addchild')
            self.assertGoodView('fiction_outlines:arcnode_create',
                                outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='left')
            self.assertInContext('form')
            self.post('fiction_outlines:arcnode_create',
                      outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='addchild',
                      data={
            ...
            self.post('fiction_outlines:arcnode_create',
                      outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='right',
                      data={
                          'arc_element_type': 'beat',
                          'description': 'This beat should be here',
                      })
            self.node_to_test.refresh_from_db()
            assert self.node_to_test.get_next_sibling().description == 'This beat should be here'
            self.post('fiction_outlines:arcnode_create',
                      outline=self.o1.pk, arc=self.arc1.pk, arcnode=self.node_to_test.pk, pos='right',
                      data={
                          'arc_element_type': 'mile_pt1',
...
class StoryNodeDetailTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:storynode_detail', outline=self.o1.pk,
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:storynode_detail', outline=self.o1.pk, storynode=self.o1_valid_storynode.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:storynode_detail',
                                outline=self.o1.pk, storynode=self.o1_valid_storynode.pk)
...
class StoryNodeCreateTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:storynode_create', outline=self.o1.pk,
                                 storynode=self.o1_valid_storynode.pk, pos='addchild')
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:storynode_create', outline=self.o1.pk,
                         storynode=self.o1_valid_storynode.pk, pos='addchild')
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:storynode_create', outline=self.o1.pk,
                          storynode=self.o1_valid_storynode.pk, pos='addchild',
                          data={
                ...
                self.post('fiction_outlines:storynode_create', outline=self.o1.pk,
                          storynode=self.o1_valid_storynode.pk, pos='right')
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.o1_valid_storynode.refresh_from_db()
    ...
    def test_authorized_user(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:storynode_create', outline=self.o1.pk,
                                storynode=self.o1_valid_storynode.pk, pos='addchild')
            self.assertGoodView('fiction_outlines:storynode_create', outline=self.o1.pk,
                                storynode=self.o1_valid_storynode.pk, pos='right')
            self.assertInContext('form')
...
class StoryNodeUpdateTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:storynode_update', outline=self.o1.pk,
                                 storynode=self.o1_valid_storynode.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:storynode_update',
                         outline=self.o1.pk, storynode=self.o1_valid_storynode.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:storynode_update',
                          outline=self.o1.pk, storynode=self.o1_valid_storynode.pk,
                          data={
                              'name': 'Chapter 1',
                ...
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                assert self.o1_valid_storynode == StoryElementNode.objects.get(pk=self.o1_valid_storynode.pk)
    ...
    def test_with_authorized_user(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.post('fiction_outlines:storynode_update', outline=self.o1.pk, storynode=self.o1_valid_storynode.pk,
                      data={
                          'name': 'Chapter 1',
                          'description': 'Updating the description',
                          'story_element_type': 'chapter',
            ...
            updated_version = StoryElementNode.objects.get(pk=self.o1_valid_storynode.pk)
...
class StoryNodeMoveTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        ...
        You have to be logged in.
        '''
        self.assertLoginRequired('fiction_outlines:storynode_move', outline=self.o1.pk,
                                 storynode=self.o1_valid_storynode.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:storynode_move', outline=self.o1.pk,
                         storynode=self.o1_valid_storynode.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:storynode_move', outline=self.o1.pk,
                          storynode=self.o1_valid_storynode.pk,
                          data={
                              '_ref_node_id': self.o1_chap2.pk,
                              '_position': 'right',
                          })
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                assert self.o1_valid_storynode.path == StoryElementNode.objects.get(pk=self.o1_valid_storynode.pk).path
    ...
    def test_valid_move(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:storynode_move', outline=self.o1.pk,
                                storynode=self.o1_valid_storynode.pk)
            self.assertInContext('form')
    ...
    def test_invalid_move(self):
        ...
        with self.login(username=self.user1.username):
            ...
            self.post('fiction_outlines:storynode_move', outline=self.o1.pk,
                      storynode=self.o1_valid_storynode.pk,
                      data={
                          '_ref_node_id': self.o1_scene1.pk,
                          '_position': 'right',
                      })
            self.response_200()
            assert len(self.get_context('form').errors) > 0
...
class StoryNodeDeleteTest(ArcNodeAbstractTestCase):
    ...
    def test_login_required(self):
        self.assertLoginRequired('fiction_outlines:storynode_delete',
                                 outline=self.o1.pk, storynode=self.o1_valid_storynode.pk)
    ...
    def test_object_permissions(self):
        ...
        for user in [self.user2, self.user3]:
            with self.login(username=user.username):
                self.get('fiction_outlines:storynode_delete',
                         outline=self.o1.pk, storynode=self.o1_valid_storynode.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
                self.post('fiction_outlines:storynode_delete',
                          outline=self.o1.pk, storynode=self.o1_valid_storynode.pk)
                self.response_302()
                assert 'accounts/login' in self.last_response['location']
    ...
    def test_normal_workflow(self):
        ...
        '''
        with self.login(username=self.user1.username):
            self.assertGoodView('fiction_outlines:storynode_delete',
                                outline=self.o1.pk, storynode=self.o1_valid_storynode.pk)
